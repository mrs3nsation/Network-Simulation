...
int main(int argc, char *argv[]){
    ...
    csma.SetQueue("ns3::DropTailQueue","MaxSize",StringValue("1000p")); //sets queue size to 1000 packets

    std::cout <<"Queue size set to 1000 packets";
    ...
    if(classifier == nullptr){
        std::cout <<"Classifier is not ipv4 format";
        return 1;
    }

    std::map<FlowId,FlowMonitor::FlowStats> stats = flowmonitor->GetFlowStats();

    std::ofstream csvFile("Metric_2.csv");
